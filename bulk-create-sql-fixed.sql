-- ===== BULK CREATE STUDENTS VIA SQL (FIXED VERSION) =====
-- Jalankan ini di Supabase SQL Editor
-- Password default: ganesha123

-- 1. Drop function lama kalau ada
DROP FUNCTION IF EXISTS bulk_create_students();

-- 2. Create function untuk bulk create users (FIXED)
CREATE OR REPLACE FUNCTION bulk_create_students()
RETURNS TABLE(student_email text, status text, new_user_id uuid) AS $$
DECLARE
...
BEGIN
  ...
  FOR student_record IN 
    ...
    FROM profiles p
    WHERE p.user_id IS NULL AND p.role = 'student'
  LOOP
    BEGIN
...
-- 6. Optional: Drop function after use
